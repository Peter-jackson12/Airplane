...
### 위 20개 조사의 증거 해석·연결을 재검토했습니다 (2026-09-18, 2회차)

1회차([위 소절](#우선-조사-대상-20개-공항의-역사적-매핑을-공식-자료로-대조했습니다-2026-09-18))가 원자료를 실제로 조회·해시한 것은 맞지만, 원문 JSON을 다시 읽어 보니 세 가지 해석·연결 문제가 있었습니다. 이번 라운드는 이 20개의 **판단만** 다시 검토했습니다 — 355개로 범위를 넓히지 않았고, 날씨 수집·stratafix 실수집·모델 재학습·다른 에이전트 실행은 하지 않았습니다.

**모집단 정정.** 이 프로젝트가 다루는 375개 공항 중 `confirmed_period` 355개는 이번에도 조사 범위 밖입니다. 본문 여러 곳에 있던 "나머지 335개"는 355의 오기로, 위 소절과 이 절 전체에서 355로 정정했습니다. 우선 조사 20개(`confirmed_current_only` 3 + `tz_conflict_needs_resolution` 8 + `unconfirmed` 9)와 `confirmed_period` 355개는 서로 겹치지 않습니다(합쳐서 375개). 또한 "20개를 조사했다"와 "20개의 역사적 신원을 확인했다"는 다른 말입니다 — 아래처럼 이번 재검토로 YUM·WRG·PSG 3곳의 판정이 확정에서 미확인/충돌 쪽으로 낮아졌으므로, 20개 모두가 확정 상태인 것은 아닙니다.

**WRG·PSG를 COOP 전용 레코드만으로 확정 처리하지 않도록 낮췄습니다.** 1회차는 IMT·KTN·PSG·SDF·SIT·WRG 6곳을 모두 같은 `facility_continuity_confirmed_tz_string_conflict_unchanged` 등급으로 묶었지만, 실제로 가져온 HOMR JSON을 보면 IMT·KTN·SDF·SIT 4곳의 레코드에는 `platforms`에 ASOS 항목이 있고 ASOS 커미션일·ACU 좌표 같은 ASOS 고유 remark가 딸려 있는 반면, **WRG(PAWG, ncdcStnId 10000446)와 PSG(PAPG, ncdcStnId 20021826)의 레코드는 `platforms`가 COOP 하나뿐이고 ASOS/AWOS 항목이 전혀 없습니다.** WRG 레코드의 "STATION INACTIVE SINCE 10/23/2012"는 이 COOP(수동 관측) 프로그램에 대한 remark이며, 이어지는 "CORRECTED LAT AND LONG TO BE AT SRG AND NOT AT AWOS"도 근처에 AWOS가 있다는 사실만 알려줄 뿐 그 AWOS 레코드 자체의 식별자·기간을 이 COOP 레코드에 연결해 주지는 않습니다. 이 COOP 레코드로 이 파이프라인이 실제로 쓰는 자동관측(ASOS/AWOS) 플랫폼의 2018~2019년 연속성을 확인했다고 쓰는 것은 근거를 넘어선 것이었습니다. 이번 재검토로 WRG·PSG 둘 다 `identity_determination`을 `unconfirmed_program_linkage_insufficient`로 낮췄고, `tz_conflict_needs_resolution` 등급(verification_tier)은 그대로입니다. IMT·KTN·SDF·SIT 4곳은 자체 레코드에 ASOS 항목이 있으므로 기존 확정 등급을 유지합니다.

**YUM을 "두 별개 관측소 확인"에서 "부분 해소, 출처 간 충돌 존재"로 낮췄습니다.** 1회차는 HOMR의 FAA:YUM 레코드(폐쇄, WSO Yuma 위치)와 KNYL 레코드(YUMA MCAS, 연속 운영)를 근거로 "서로 다른 두 관측소"라고 확정했습니다. 이번에 KNYL(ICAO_KNYL.json)의 경로·URL·해시·레코드 식별자(NOAA HOMR ncdcStnId 20000934)를 증거표에 실제로 연결하면서 다시 읽어 보니 두 가지를 추가로 구분해야 했습니다. 첫째, KNYL 레코드의 `platforms`에는 ASOS가 있지만 이 항목은 2021-12-16자 ad hoc 메타데이터 갱신("ADDING ASOS PLATFORM PER INFORMATION IN ASOS LOCATION XLS PROVIDED BY NWS CM OFFICE")으로 추가된 것이고, POR 1960-07-01~Present는 COOP 계열의 기록 보유 기간입니다 — 2021년 메타데이터 추가일을 실제 ASOS 설치일로 단정하지 않았고, 1960년 이후 전체를 ASOS 운영기간으로 단정하지도 않았습니다. 둘째, 이 프로젝트가 이미 캐시해 둔 IEM `AZ_ASOS` 네트워크 파일(`data/weather_probe/networks/AZ_ASOS.geojson`) 자체에 **세 번째 레코드**로 feature `YUM`이 있는데, 좌표가 HOMR의 FAA:YUM 레코드(32.66667/-114.6, 옛 기상청 사무소)가 아니라 feature `NYL`과 같은 좌표(-114.606/32.6566, Yuma MCAS)이고 `ncei91`/`climate_site` 코드도 NYL과 동일합니다. 즉 HOMR과 이 프로젝트가 이미 갖고 있는 IEM 자체 캐시가 폐쇄된 "YUM" 식별자의 물리적 위치에 대해 서로 다른 말을 하고 있으며, 이번에도 이 충돌을 풀지 않았습니다. 두 출처가 공통으로 지지하는 것은 "2018~2019년을 연속으로 덮는 것은 `NYL`이다"라는 점뿐입니다. `identity_determination`을 `identifier_mismatch_partially_resolved_source_conflict`로 낮췄습니다.

**AZA처럼 한 조회에 여러 레코드가 나오는 경우, 실제로 쓴 레코드를 명시했습니다.** ICAO:KIWA 조회는 AWOS 레코드(ncdcStnId 10000826, WBAN 23104, Phoenix-Mesa Gateway)와 무관한 NEXRAD 레코드(ncdcStnId 30001870)를 함께 반환합니다. 1회차 서술은 어느 쪽을 근거로 썼는지 밝히지 않았습니다 — 이번에 판단에 쓴 것이 AWOS 레코드(10000826)뿐이고 NEXRAD 레코드는 쓰지 않았음을 증거표의 `program_location_linkage` 필드에 명시했습니다.

**ISN/XWA는 유지하되 경계를 명시했습니다.** ISN→XWA 시설 이전이라는 결론 자체(HOMR의 명시적 "NOT CONSIDERED CLIMATOLOGICALLY COMPATIBLE" remark)는 바뀌지 않았습니다. 다만 ISN의 HOMR 레코드 종료일(2024-02-27)은 레코드 자체의 메타데이터 경계이고, "LAST LCD PUBLICATION SEPT 2019" remark나 IEM의 archive_end(2019-10-18, 기존 매핑 작업 기준)와는 다른 종류의 날짜라는 점, 그리고 이 셋 중 어느 것도 FAA 시설 폐쇄일을 직접 증명하지 않는다는 점을 증거표에 구분해 남겼습니다.

**STT/STX 시간대 정책, SPN 수집 경로는 기존 그대로 미해결로 남깁니다.** 두 항목 다 1회차의 결론(정황 증거만 있고 등급은 올리지 않음, SPN은 IEM `GU_ASOS` 캐시의 공백이지 관측소 부재의 증거가 아님)을 다시 검토했지만 바꿀 근거가 없어 그대로 유지했습니다.

**"17곳은 식별자 보정만 하면 충분하다"는 일괄 결론을 걷어냈습니다.** 위 수정 결과 20개는 이제 항목별로 다른 근거 수준을 가집니다 — 식별자 대체로 해소된 8곳(AZA·BKG·FCA·HHH·MQT·PBI·SCE·USA)과 자체 ASOS remark로 시설 연속성이 확인된 4곳(IMT·KTN·SDF·SIT)은 근거가 상대적으로 탄탄하지만, ISN·XWA(경계 표시 추가)·YUM(출처 충돌 미해결)·STT·STX(시간대 미해결)·SPN(네트워크 공백)·WRG·PSG(프로그램 연결 부족)는 각각 다른 이유로 미확인이거나 추가 확인이 필요합니다. 20개를 하나의 뭉뚱그린 결론으로 요약하지 않습니다.

**재현.** [notebooks/verify_priority_station_identity.py](notebooks/verify_priority_station_identity.py)에 `--cache-dir`(기존 캐시 재사용, 기본값은 1회차가 채운 `data/weather_probe/baseline_recovery_v2_station_identity_investigation_20260918_homr_raw/`)와 `--allow-network`(명시하지 않으면 캐시에 없는 조회는 네트워크로 전환하지 않고 즉시 실패)를 추가했습니다. 원자료를 두 개 이상 연결해야 하는 항목(YUM은 FAA:YUM과 ICAO:KNYL 둘 다)은 `HOMR_EXTRA_QUERY`로 추가 조회를 같은 증거 행에 연결하고, IEM 교차 확인은 `IEM_FEATURE_CHECK`로 실제 사용한 네트워크 파일·feature id·해시·archive 요약을 증거표 `iem_cross_check` 열에 기록합니다(네트워크 이름만 남기지 않습니다). 캐시 파일의 재조회일은 파일시스템 mtime 추정치로만 기록하고(`raw_evidence_fetch_date_basis=filesystem_mtime_estimate_not_a_verified_fetch_log`) 이번 실행의 검토일(`evidence_reviewed_date`)과 분리했습니다 — 캐시 재사용일을 최초 조회일로 표시하지 않습니다.

```powershell
.venv/Scripts/python.exe -u -m notebooks.verify_priority_station_identity --name baseline_recovery_v2_station_identity_review_20260918
```

새 [증거표](output/baseline_recovery_v2_station_identity_review_20260918_evidence.csv)와 [manifest](output/baseline_recovery_v2_station_identity_review_20260918_manifest.json)를 남겼습니다. 1회차의 [증거표](output/baseline_recovery_v2_station_identity_investigation_20260918_evidence.csv)·[manifest](output/baseline_recovery_v2_station_identity_investigation_20260918_manifest.json)·원자료 캐시는 덮어쓰지 않고 그대로 보존했습니다. 이 재현은 사람이 내린 판단(FINDINGS)을 다시 조립하는 것이며, 공식 자료 자체의 완전성을 자동으로 증명하지 않습니다 — 미확인으로 남은 항목은 이 라운드가 연결 근거를 찾지 못했다는 뜻이지, 그런 근거가 어디에도 없다는 뜻이 아닙니다. `tests/test_verify_priority_station_identity.py`(20개 테스트: 대상 집합의 20/355 비중첩, 원자료 존재·해시, YUM의 두 레코드 연결, WRG/PSG의 ASOS 미보유 확인, IMT/KTN/SDF/SIT의 ASOS 보유 확인, AZA의 ncdcStnId 특정, SPN의 GU_ASOS 미검출, 캐시 전용 모드의 네트워크 차단과 기존 파일 불변)을 새로 추가해 전체 회귀 테스트가 354개에서 **374개 통과**로 늘었습니다. `historical_identity_confirmed`/`verification_tier`를 포함한 기존 production 매핑·선정 결과·캐시·재결합 결과는 이번에도 변경하지 않았습니다.

### 위 20개 조사의 현재 매핑과 역사적 연속성을 분리하고, 원자료 검증을 실행 경로에 연결했습니다 (2026-09-18, 3회차)

새 외부 자료·새 IEM 조회는 이번에도 하지 않았습니다. 기존 캐시와 2회차 텍스트를 다시 검토해 두 가지만 고쳤습니다 — 355개로 범위를 넓히지 않았고, 날씨 수집·stratafix 실수집·모델 재학습·다른 에이전트 실행은 하지 않았습니다. production `mapping_table.csv`·`verification_tier`·collectible·stratafix 300행의 ID·순서·분모도 이번에도 변경하지 않았습니다.

**1) 현재 매핑 확인과 2018~2019 역사적 연속성 확인을 분리했습니다.** 2회차의 `_TZ_ALIAS_CONFIRMED` 4곳(IMT·KTN·SDF·SIT)은 모두 같은 `facility_continuity_confirmed_tz_string_conflict_unchanged` 등급을 받았지만, 실제로는 KTN만 날짜가 명시된 과거 사건(1997년 "COMPATIBLE STATION MOVE"로 ASOS 커미션)을 갖고 있고, IMT·SDF·SIT는 현재 ASOS 플랫폼·ACU 좌표·(SIT의 경우) 날짜 없는 "REVIEWED. NO KNOWN CHANGES." 메모뿐입니다. POR이 "Present"까지 이어진다는 것과 이전 remark가 없다는 것은 **현재** 시설·프로그램 연결의 근거일 뿐, 2018~2019년이라는 특정 구간을 직접 뒷받침하지 않습니다(1997년 사건조차 그 구간보다 21년 앞선 것이어서, 그 자체로 2018~2019년의 직접 증거는 아닙니다). 이번 라운드는 IMT·SDF·SIT를 `current_facility_confirmed_historical_continuity_unconfirmed_tz_string_conflict_unchanged`로 낮추고, KTN은 날짜가 명시된 1997년 사건을 근거로 별도 등급(`facility_continuity_supported_by_dated_1997_event_tz_string_conflict_unchanged`)을 유지합니다. 두 등급 모두 "현재 매핑이 틀렸다"는 뜻이 아니라 "2018~2019년 연속성이 별도로 확인되지 않았다"는 뜻입니다. 20개 전 항목에 `historical_continuity_2018_2019` 필드를 새로 추가해 이 구분을 일관되게 남겼습니다.

같은 원칙을 식별자 불일치 8곳(AZA·BKG·FCA·HHH·MQT·PBI·SCE·USA)에도 적용했습니다. 2회차는 8곳 모두 `inference_and_unresolved`를 "none beyond remaining_gap below."로 남겼는데, 이는 POR과 "이전 remark 부재"만 있는 6곳(AZA·BKG·HHH·MQT·SCE·USA)에서 침묵을 긍정적 근거처럼 보이게 했습니다. 이번에 FCA(2005-10-25 개명이 날짜로 명시돼 연구 기간 이전에 이미 확정)·PBI(2018~2019년 당시 옛 식별자로 계속 운영됐고 개명이 연구 기간 **이후**라고 레코드가 직접 서술)는 시기가 뒷받침되는 근거로 구분하고, AZA(개명 remark 자체가 "DATE UNKNOWN")와 BKG·HHH·MQT·SCE·USA(remark 부재만 있음)는 "현재 대체 ID 대응은 확인됐으나 2018~2019년 연속성은 remark 부재로부터의 추론일 뿐 별도로 확인되지 않았다"고 명시했습니다. 현재 대체 ID가 같은 시설을 가리킨다는 결론 자체는 바뀌지 않았습니다(identity_determination은 8곳 모두 `identifier_mismatch_resolved_same_facility` 유지).

**2) 원자료 레코드 검증을 실제 생성 경로(`main()`)에 연결했습니다.** 기존 `fetch_homr`는 캐시 파일의 해시만 계산하고, `main()`은 고정된 `FINDINGS` 텍스트를 그대로 표에 썼습니다 — 캐시 파일이 존재하고 해시가 찍힌다는 것과 그 안의 레코드가 실제로 `FINDINGS`의 주장을 뒷받침한다는 것은 별개입니다. 새 `EXPECTED_RECORD`가 각 판단이 인용하는 정확한 `ncdcStnId`와 그 레코드가 반드시 갖거나(예: IMT는 ASOS 필수) 갖지 않아야 할(예: WRG·PSG는 ASOS/AWOS 금지) `platforms`를 구조화해 지정하고, `load_station_collection`·`select_expected_station`·`verify_expected_record`가 이번 실행에서 실제로 받은 JSON에서 그 레코드를 선택해 대조합니다. AZA는 같은 조회(ICAO:KIWA)가 반환하는 AWOS 레코드(10000826)와 NEXRAD 레코드(30001870)를 `ncdcStnId`로 각각 선택해 서로 다른 조건으로 검사하고, YUM은 FAA:YUM(20000933, ASOS 없어야 함)과 ICAO:KNYL(20000934, ASOS 있어야 함) 두 파일을 각각 검사합니다. `IEM_FEATURE_CHECK`도 SPN을 제외한 모든 항목이 `found=True`여야 하도록 요구합니다(SPN의 미검출은 애초에 `FINDINGS['SPN']`이 `unconfirmed`이므로 유일한 허용된 예외입니다). 필수 레코드가 없거나, 선택된 레코드의 플랫폼이 기대와 충돌하거나, 필수 IEM feature가 빠지면 `main()`이 `RuntimeError`로 실행 전체를 중단하고 증거표·manifest를 아예 쓰지 않습니다 — "원자료 파일이 존재한다"와 "그 레코드가 여전히 서술을 뒷받침한다"를 둘 다 요구하는 명시적 실패 정책입니다.

**검증.** 네트워크 호출을 막은 상태에서(`--allow-network` 미지정) 다음을 회귀 테스트로 확인했습니다: 캐시가 `{}`이거나 `stations`가 빈 경우 `load_station_collection`이 예외를 냄, 선택하려는 `ncdcStnId`가 응답에 없는 경우 검증 실패로 보고됨, 필요한 플랫폼이 제거되거나 금지된 플랫폼으로 바뀐 캐시로는 `main()`이 `RuntimeError`를 내고 증거표·manifest를 쓰지 않음(임시로 캐시를 변조한 사본에 대해서만; 원본 캐시는 손대지 않음), AZA의 두 레코드와 YUM의 이중 자료 연결이 실제 캐시로 정상 통과함, IEM feature가 없으면(SPN 제외) 검증이 실패함, SPN의 예상된 미검출은 예외로 여전히 통과함, 정상 캐시로 새 실행명(`baseline_recovery_v2_station_identity_verification_20260918`)이 재현되고 기존 산출물·원자료는 바뀌지 않음. 전체 스위트는 **374개에서 395개 통과**로 늘었습니다.

```powershell
.venv/Scripts/python.exe -u -m notebooks.verify_priority_station_identity --name baseline_recovery_v2_station_identity_verification_20260918
.venv/Scripts/python.exe -u -m pytest -q
```

새 [증거표](output/baseline_recovery_v2_station_identity_verification_20260918_evidence.csv)와 [manifest](output/baseline_recovery_v2_station_identity_verification_20260918_manifest.json)를 남겼습니다. 1·2회차의 증거표·manifest·원자료 캐시는 이번에도 덮어쓰지 않고 보존했습니다. **정리.** 이번 라운드가 끝난 뒤 20개는 두 가지 별도 결론으로 나뉩니다 — (a) *현재 식별자 대응*: 20개 전부 현재 IEM/HOMR 레코드가 어떤 시설을 가리키는지는 확인됐습니다(YUM만 HOMR·IEM 자체 캐시 간 좌표 충돌이 남아 있어 부분적입니다). (b) *역사적 적용기간(2018~2019)*: ISN·XWA(날짜가 명시된 이전/신규 개시 사건으로 구간 일부를 직접 뒷받침)·FCA(날짜 명시 remark로 구간을 뒷받침)·PBI(**정정 — 아래 4회차 참고**: 이 문장이 쓰였을 때는 "날짜/명시적 서술로 구간을 뒷받침"이라고 했지만, 실제로는 원자료 remark에 그런 날짜 서술이 없어 4회차에서 하향했습니다)·KTN(1997년 사건이 있으나 구간을 직접 뒷받침하지는 않음)을 제외한 나머지(IMT·SDF·SIT·STT·STX·AZA·BKG·HHH·MQT·SCE·USA·YUM·SPN·WRG·PSG)는 2018~2019년 연속성이 미확인 또는 제한된 근거 상태로 남아 있습니다. 이 구분은 후속 코드 작업(예: `map_weather_stations.py`의 candidate_sid 수정)이 "현재 식별자를 어떻게 바꿀지"와 "그 변경이 2018~2019년에 실제로 유효했는지"를 따로 다뤄야 한다는 뜻입니다.

### 위 20개 조사의 PBI 원자료 귀속 오류와 식별자 검증 공백을 수정했습니다 (2026-09-18, 4회차)

새 외부 자료·새 IEM 조회는 이번에도 하지 않았습니다. 기존 캐시(`data/weather_probe/baseline_recovery_v2_station_identity_investigation_20260918_homr_raw/FAA_PBI.json` 포함)와 이 파일 자체의 서술을 다시 검토해 세 가지만 고쳤습니다 — 355개로 범위를 넓히지 않았고, 날씨 수집·stratafix 실수집·모델 재학습·다른 에이전트 실행은 하지 않았습니다. production `mapping_table.csv`·`verification_tier`·collectible·stratafix 300행의 ID·순서·분모도 이번에도 변경하지 않았습니다.

**1) PBI의 "개명이 연구 기간 이후"라는 서술이 원자료 remark가 아니라 배경지식이었습니다.** 위 3회차까지 `_IDENTIFIER_MISMATCH['PBI']`와 `_CONTINUITY_TEXT`는 HOMR remark가 "2018~2019년 당시 옛 식별자로 운영됐고 개명이 연구 기간 이후"라고 명시하는 것처럼 서술했습니다. 그러나 실제로 캐시된 `FAA_PBI.json`의 `remarks` 배열에는 그런 날짜 서술이 전혀 없습니다 — 있는 것은 현재 식별자(FAA=DJT, ICAO=KDJT, NWSLI=PBI 유지, POR 1938-07-01~Present, 이전 remark 없음)와 `updates` 블록의 미날짜 성격 항목(`"UPDATING STATION NAME PER FAA (AIRNAV)"`, enteredDate 2026-07-15, AD HOC 갱신)뿐입니다. 이 갱신일은 메타데이터 수정일이지 실제 개명일이 아니며, "개명이 2018~2019 이후"라는 것은 이 프로젝트가 원자료 밖에서 갖고 있던 배경지식(2025년 실제 FAA/법적 개명)이 remark 인용인 것처럼 섞여 들어간 것이었습니다. `historical_continuity_2018_2019`를 `explicit_statement_change_postdates_window`에서 `rename_confirmed_by_current_ids_undated_in_record`로 낮췄습니다 — 현재 식별자 대응은 여전히 확인되지만, 그 변경이 언제 일어났는지는 이 레코드가 직접 말하지 않습니다. FCA의 실제 2005년 개명 remark와 KTN의 실제 1997년 사건은 원자료에 날짜가 명시돼 있어 이번에도 그대로 유지했습니다.

**2) 필요한 HOMR 식별자 검증을 실제 생성 경로에 추가했습니다.** 기존 `EXPECTED_RECORD`/`verify_expected_record()`는 선택된 레코드의 `ncdcStnId`와 `platforms`만 대조했고, `identifiers`는 검사하지 않았습니다 — 즉 `ncdcStnId`와 `platforms`를 그대로 둔 채 `identifiers`만 삭제되거나 다른 FAA/ICAO/NWSLI/NEXRAD 값으로 바뀌어도 기존 FINDINGS 텍스트가 그대로 출력될 수 있었습니다. 새 `EXPECTED_IDENTIFIERS`(ncdcStnId별 필수 idType/id 쌍)와 `verify_expected_record()`에 추가한 검사가 이제 선택된 레코드의 `identifiers`에 그 쌍이 실제로 있는지 확인하고, 누락되면 증거표·manifest 작성 전에 `RuntimeError`로 실행을 중단합니다. 모든 HOMR 조회 키가 레코드 자신의 현재 식별자와 같아야 한다고 강제하지는 않습니다 — 예를 들어 PBI는 `FAA:PBI`로 조회하지만(NOAA 검색 색인이 여전히 인식하는 과거 키) 레코드 자신의 현재 FAA 식별자는 DJT이며, 이 둘이 다른 것은 정상입니다. AZA의 AWOS(10000826)/NEXRAD(30001870) 두 레코드, YUM의 FAA:YUM(20000933)/ICAO:KNYL(20000934) 두 파일은 각각 다른 식별자 쌍으로 따로 검사합니다.

**3) 새 산출물의 모순된 설명을 정정했습니다.** manifest limitations의 "PSG와 WRG는 하향됐지만 IMT/KTN/SDF/SIT는 facility_continuity_confirmed를 유지한다"는 2회차 시점 서술이 이후 3회차의 IMT/SDF/SIT 하향과 모순된 채로 새 manifest에 그대로 남아 있었습니다 — 이번에 이 문장을 3회차의 실제 판정(KTN만 별도 등급 유지, IMT/SDF/SIT는 하향)과 맞춰 정정했습니다.

**검증.** 네트워크 호출을 막은 상태에서(`--allow-network` 미지정) 다음을 새 회귀 테스트로 확인했습니다: PBI 원자료(`FAA_PBI.json`)의 `remarks`에 DJT/TRUMP/RENAME 계열 문구나 날짜 명시 개명 서술이 없음, `verify_expected_record()`가 필수 식별자 누락·변경을 각각 별도 실패로 보고함, `identifiers`만 삭제되거나 FAA 값만 변경된 사본으로는 `ncdcStnId`·`platforms`가 그대로여도 `main()`이 `RuntimeError`를 내고 증거표·manifest를 쓰지 않음(임시 사본에 대해서만; 원본 캐시는 손대지 않음), `EXPECTED_IDENTIFIERS`가 `EXPECTED_RECORD`의 22개 검사와 정확히 같은 `ncdcStnId` 집합을 덮음. 기존 20/355 비중첩·원자료 해시·YUM 이중 연결·WRG/PSG의 ASOS 미보유·IMT/KTN/SDF/SIT의 ASOS 보유·AZA의 ncdcStnId 특정·SPN의 GU_ASOS 미검출·캐시 전용 모드 네트워크 차단 등 기존 회귀 테스트는 모두 그대로 통과했습니다. 전체 스위트는 **395개에서 402개 통과**로 늘었습니다.

```powershell
.venv/Scripts/python.exe -u -m notebooks.verify_priority_station_identity --name baseline_recovery_v2_station_identity_verification_fix_20260918
.venv/Scripts/python.exe -u -m pytest -q
```

새 [증거표](output/baseline_recovery_v2_station_identity_verification_fix_20260918_evidence.csv)와 [manifest](output/baseline_recovery_v2_station_identity_verification_fix_20260918_manifest.json)를 남겼습니다. 1~3회차의 증거표·manifest·원자료 캐시는 이번에도 덮어쓰지 않고 보존했습니다. **한계.** 이 라운드는 PBI의 원자료 귀속 경계와 식별자 검증 공백만 좁혔습니다 — PBI를 포함해 20개 중 이번에 새로 확정된 항목은 없고, YUM의 HOMR·IEM 좌표 충돌, SPN의 네트워크 공백, STT/STX의 시간대 미해결, WRG/PSG의 프로그램 연결 부족은 이번에도 그대로 미해결로 남아 있습니다. `historical_identity_confirmed`/`verification_tier`를 포함한 기존 production 매핑·선정 결과·캐시·재결합 결과는 이번에도 변경하지 않았습니다.
...
- **남은 것.** 채택된 706,759행 전체에 대한 날씨 실수집과 재학습은 이번에도 수행하지 않았습니다. **12개월을 모두 검사한 뒤 날짜 귀속을 보류한 293,241행**(결측 키 292,683행 + 완전 지문이지만 후보 연도 0개·2개인 558행)은 "검사하지 않은 행"이 아니라 "검사했지만 채택 조건을 충족하지 못해 보류한 행"입니다 — 미검사 월은 이제 없습니다. 매핑 미확인·시간대 충돌 15,373행(2.09%)은 원인을 먼저 해결해야 전체 결합 대상에 넣을 수 있습니다. 전체 결합 전에 이번 검증에서 확인한 경계 규칙(같은 컷오프, 동일 관측 나이 제한, DST 정책)을 그대로 확장 적용해야 합니다. **전체 375개 공항(`confirmed_period` 355 + 우선 조사 20) 중 우선 조사 대상 20개(`confirmed_current_only` 3 + `tz_conflict_needs_resolution` 8 + `unconfirmed` 9)만 NOAA/NCEI HOMR로 검토했고, 나머지 `confirmed_period` 355개는 이번에도 조사 범위 밖입니다.** "조사를 마쳤다"와 "역사적 신원 확인이 완료됐다"는 구분됩니다 — 20개 모두 HOMR 조회는 마쳤지만, 그중 YUM·SPN·WRG·PSG·STT·STX 6곳은 여전히 미확인이거나 두 공식 자료 사이의 충돌이 남아 있어 신원이 확정되지 않았습니다(2회차 재검토, 아래 소절 참고). 확정 상태로 만드는 것 자체가 목표가 아니며, 근거가 부족한 항목은 미확인으로 남기는 편이 정확합니다. 자세한 내용과 남은 공백은 2절의 두 소절과 [증거표](output/baseline_recovery_v2_station_identity_review_20260918_evidence.csv)를 참고하세요. **이번 조사도 IEM 메타데이터의 현재 목록을 기준점으로 삼되 NOAA/NCEI HOMR라는 별도 공식 자료로 교차 확인했다는 점에서 이전 라운드(IEM 현재 좌표·시간대 동치성만 확인)보다 진전됐지만, FAA 공항 마스터 레코드(예: 날짜가 명시된 Form 5010)는 이번에도 조회하지 못했고, 확인한 8곳의 대체 식별자를 실제로 매핑 코드에 반영하지도 않았습니다.** `confirmed_period` 355개 공항과 위 미해결 항목을 다음 단계로 제안합니다 — 이번에 확인한 20개 결과가 이 355개에 그대로 일반화되지는 않습니다. **새 규칙으로 다시 선정한 300행 표본(`baseline_recovery_v2_weather_expanded_stratafix_20260918`)은 이번에도 실제로 수집·결합하지 않았습니다** — 선정만 했고, 비교 기준은 여전히 기존 300행입니다. 신규로 필요한 구간(약 220만 station-시간)의 실제 수집도 이번 범위에서는 하지 않았습니다. [기상 결합 재검토(2026-09-16)](output/weather_recovery_review.md), [달력 분석](notebooks/analyze_calendar_signature.py)
...
기본은 **캐시 전용**입니다 — 인터넷 접속이 필요 없고, 1회차가 채운 `data/weather_probe/baseline_recovery_v2_station_identity_investigation_20260918_homr_raw/`를 그대로 재사용합니다. 새 이름으로 재실행해도 이 캐시나 1회차 증거표·manifest는 덮어쓰지 않습니다. 캐시에 없는 조회가 있으면 네트워크로 넘어가지 않고 즉시 실패합니다(`--allow-network`를 명시해야 네트워크 조회를 허용). 재학습이 없고 타깃·지연·실제 출도착 시각·날씨 값을 읽지 않습니다.

```powershell
.venv/Scripts/python.exe -u -m notebooks.verify_priority_station_identity --name <새로운-이름>
```

기존 캐시가 아닌 다른 캐시 디렉터리를 쓰려면 `--cache-dir <경로>`를 추가합니다. 대상 20개가 `output/baseline_recovery_v2_weather_scope_fix_20260918_mapping_priority_investigation.csv`의 iata 집합과 정확히 일치하지 않으면 예외로 중단되고, `<새로운-이름>`이 기존 출력과 겹치면(이미 `_evidence`/`_manifest` 파일이 있으면) 예외로 중단됩니다. 3회차부터는 `main()`이 fetched HOMR JSON의 `stationCollection.stations`에서 각 판단이 인용하는 `ncdcStnId`를 실제로 선택해 그 레코드의 `platforms`가 기대(요구/금지)와 맞는지, 그리고 SPN을 제외한 모든 `IEM_FEATURE_CHECK` 항목이 `found=True`인지를 확인합니다 — 하나라도 맞지 않으면 `RuntimeError`로 중단되고 증거표·manifest를 쓰지 않습니다. `tests/test_verify_priority_station_identity.py` 회귀 테스트는 2회차 20개에서 3회차 41개로 늘었고(전체 스위트 395개 통과), 코드를 실제로 바꾼 라운드이므로 1회차의 "코드·collectible 변경 없음" 서술은 2·3회차 모두 적용되지 않습니다.
...
| 우선 조사 대상 20개 공항의 역사적 매핑을 공식 자료로 어떻게 확인했는가 | 1회차 [증거표](output/baseline_recovery_v2_station_identity_investigation_20260918_evidence.csv)/[manifest](output/baseline_recovery_v2_station_identity_investigation_20260918_manifest.json), 2회차 재검토 [증거표](output/baseline_recovery_v2_station_identity_review_20260918_evidence.csv)/[manifest](output/baseline_recovery_v2_station_identity_review_20260918_manifest.json), 3회차(현재 매핑/역사적 연속성 분리, 원자료 검증을 생성 경로에 연결) [증거표](output/baseline_recovery_v2_station_identity_verification_20260918_evidence.csv)/[manifest](output/baseline_recovery_v2_station_identity_verification_20260918_manifest.json), [재현 코드](notebooks/verify_priority_station_identity.py) |
