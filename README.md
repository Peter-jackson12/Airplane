...
### Marketing Carrier 자료로 11월이 전부 설명됐습니다 (2026-09-17)
...
### BTS 11월 Marketing Carrier 대조

보고 범위 설명의 직접 검증입니다. Marketing ZIP 두 개가 `data/bts/`에 있어야 하며 재학습이 없습니다.

```powershell
uv run --offline python -u notebooks/verify_bts_november_marketing.py --name baseline_recovery_v2_bts_november_marketing_20260917
```
...
이번 작업의 코드 검증은 **210개 테스트 한 번의 실행에서 전부 통과**입니다. 기존 193개와 BTS 대조 4개에 진단 검사 7개, Marketing 대조 검사 6개를 더한 것입니다. Marketing 검사는 운항사/판매 항공사 대응 고정, 지연·타깃 열 미사용, 공동운항으로 중복된 판매 기록이 조용히 합쳐지지 않음을 포함합니다. 추가 검사는 완화 사다리의 모든 단계가 타깃·지연 열을 쓰지 않음, 키를 빼면 후보 수가 줄지 않음(단조성), 거리만 다른 행이 거리 제외 단계에서만 후보가 됨, 필드 차이 진단이 가장 가까운 후보를 남김, 행별 결측 키 패턴이 실제 관측된 키와 정확히 일치함을 포함합니다.
