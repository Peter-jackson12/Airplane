...
사용자는 날씨 추가 성능 실험까지 진행하고 GitHub README 형식으로 제출하기로 했습니다. 다음 순서로 마무리합니다.

1. **기존 작업 환경 확인:** 원본 BTS·관측소·날씨 캐시와 행별 결과가 남아 있는 컴퓨터에서 해시·실행 상태를 확인합니다. 이 PC의 파일 누락을 기존 분석 미완료로 해석하지 않으며, 검증된 기존 결과를 불필요하게 재생성하지 않습니다.
2. **날씨 결합 자격과 표본 검증:** 필요한 관측소 매핑·역사적 적용기간의 불확실성을 처리하고 수정된 stratafix 300행 표본을 실제 수집·결합합니다. 근거가 부족한 항목은 보류·결측으로 유지하고 그 범위를 기록합니다.
3. **전체 대상 결합:** 날짜 귀속을 채택한 706,759행을 대상으로 예측 시점·가용성·관측 나이·매핑 규칙에 따라 날씨를 수집·결합하고, 결합률·결측·제외 사유를 검증합니다. 모든 행의 날씨가 반드시 채워져야 한다는 뜻은 아닙니다.
4. **날씨 유무 성능 비교:** 같은 라벨 평가 행·분할·시드·nested 프로토콜로 기준선과 날씨 추가 조건을 재학습·비교합니다. 아래 정보 경계를 유지하고 성능 향상 여부, 시드별 차이와 한계를 보고합니다.
5. **GitHub 제출 마무리:** README에 최종 결과·재현 명령·필요 데이터와 확보 방법·근거 링크·한계를 통합하고, 코드와 실행 기록의 정합성을 확인해 제출합니다. DOCX 변환, 모델 서빙, 독립 미래 운영 검증은 이번 제출의 필수 완료 조건이 아닙니다.

현재는 2~4번의 날씨 확장 작업이 남아 있으며, 기존 전처리·BTS 분석의 완료 결과는 유지합니다. 성능이 개선되지 않더라도 적절한 비교 실험을 완료하고 그 결과를 명확히 설명하면 이번 검증 목표를 마무리할 수 있습니다.
...
**날씨 유무 비교의 평가 계약(설계 완료, 실제 비교 재학습은 아직 미실행).** 날씨 유무 성능을 비교할 때는 다음을 지켜야 합니다.
...
**최종 제출은 GitHub 저장소의 README를 중심으로 합니다.** 아래 Word 양식과 과거 보고서 초안은 참고 자료로 보존하며, 별도 요청이 없으면 DOCX 작성·변환이나 양식 맞춤을 수행하지 않습니다. README에서 최종 결론과 재현 가능한 실행 근거를 함께 확인할 수 있도록 정리합니다.

| 문서 | 용도 |
|---|---|
| **이 README** | GitHub 최종 제출물의 중심. 전체 설명·발표 흐름·최신 결론·근거 안내 |
| [양식에 맞춘 보고서 초안](output/preprocessing_current_report.md) | 2026-09-16 시점의 과거 초안. 현재 제출 필수 산출물이 아님 |
| [원본 Word 양식](머신러닝%20모델링%20프로젝트%20보고서%20템플릿.docx) | 참고용 보존. 현재 제출에서 사용 의무 없음 |
