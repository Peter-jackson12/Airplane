...
작성: Peter-jackson12 TF · 갱신: 2026-09-17 · 문서 버전: v0.11
...
| 날짜 달력 구조 확인 | 공휴일 신호로 실제 미국 달력임을 확인하고 최소 두 해가 섞였음을 관측 |
...
행 수는 파일의 구성에 대한 증거이지 실제 운항량에 대한 증거가 아니며, 공휴일 신호는 관측된 기술이지 인과의 증명이 아닙니다. [재현 코드](notebooks/analyze_calendar_signature.py), [일별 분해](output/baseline_recovery_v2_calendar_signature_20260917_daily.csv), [추수감사절 후보](output/baseline_recovery_v2_calendar_signature_20260917_thanksgiving.csv), [실행 지문](output/baseline_recovery_v2_calendar_signature_20260917_manifest.json)
...
1. **날씨 결합은 현재 모델에서 제외했고, 이번에 장벽이 하나 더 확인됐습니다.** 위 2절의 달력 분석에서 **이 파일이 최소 두 해가 섞인 자료**임이 관측됐습니다. 따라서 "연도 하나를 확정해 전체에 날씨를 붙인다"는 기존 계획은 성립하지 않습니다. 사건별 날씨를 붙이려면 **행마다 연도를 판정**해야 하고, 그러려면 BTS 원본 대조를 후보 연도별로 수행해 행 단위로 귀속시켜야 합니다. 후보는 요일 배치 기준 2007·2018 계열과 2013·2019 계열입니다. 날씨가 쓸모없다는 결론과는 다릅니다. [기상 결합 재검토](output/weather_recovery_review.md), [달력 분석](notebooks/analyze_calendar_signature.py)
