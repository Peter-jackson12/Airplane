"""Re-compare complete November inputs with the BTS Marketing Carrier files.

Tests the reporting-scope explanation for the 5,494 rows that the Reporting
Carrier files could not match. Diagnostic only: no target columns, no
imputation, no model fitting, no year assignment. Missing-key rows are reported
separately and never pooled with complete-fingerprint rows. Row evidence stays
in data/ and is excluded from Git.
"""
...
def read_marketing(year: int) -> tuple[pd.DataFrame, Path]:
    path = ROOT / ('data/bts/On_Time_Marketing_Carrier_On_Time_Performance_Beginning'
                   f'_January_2018_{year}_11.zip')
    ...
    assert frame.Year.eq(year).all() and frame.Month.eq(11).all()
    return normalize(frame.rename(columns=RENAMES)), path


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument('--name', default='baseline_recovery_v2_bts_november_marketing_20260917')
    args = ap.parse_args()
    ...
    incomplete = november.loc[november[KEYS].isna().any(axis=1)].copy().reset_index(drop=True)

    # the rows the Reporting Carrier files could not match, from the committed evidence
    ...
    cache = {}
    for year in YEARS:
        frame, path = read_marketing(year)
        cache[year] = frame
        ...
        matched = counts > 0
        summary.append({'year': year, 'source': 'marketing', 'eligible_rows': len(complete),
    ...
    # ---- did the previously unmatched rows recover? ----
    ...
    manifest = {
        ...
        'november_rows': int(len(november)), 'complete_rows': int(len(complete)),
        ...
        'reporting_unmatched_now_matched': int(((~pd.Series(prior_hit)) & any_hit.to_numpy()).sum()),
        ...
        'limitations': [
            'Only November 2018 and 2019 are covered; no other month or year is tested.',
